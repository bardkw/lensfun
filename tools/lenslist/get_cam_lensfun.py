...
class Lens:
    def __init__(self, element, root, camtype):
        ...
        if self.distortion:
            self.all_distortions = []
            for dist in element.findall("calibration/distortion"):
                self.all_distortions.append([dist.get('model'), dist.get('focal'), dist.get('a'),dist.get('b'),dist.get('c'), dist.get('k1'), dist.get('k2'), dist.get('k3')])
                if dist.get('model') != 'poly3' and dist.get('model') != 'ptlens':
                    print(f"different model: {dist.get('model')}")
...
if cmdline_args['git']==True:
    ...
    subprocess.check_output(["git", "clone", "https://github.com/lensfun/lensfun.git", XmlDBPath])
